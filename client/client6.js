SERVER_ADD="http://128.208.49.41:8080/";
...
var child = spawn('python',["./VolumePitchMeter.py"] ); 

child.stdout.on('data', function(data) {
	var split = (data.toString()).split(","); 
	if(socket.connected)
		socket.emit('responseFromDataClient', JSON.stringify({ dataFromDataClient: split[0], pitchFromDataClient: split[1], clientid: id}));
	//console.log('Sent to server: ' + split[0] + "," + split[1]); //Print data sent to server
});
