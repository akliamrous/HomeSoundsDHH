...
httpListener.sockets.on('connection', function (socket) {
    ...
    socket.on('responseFromDataClient', function (data) {
        ...
        //Waveform
        socket.broadcast.to('http-waveform').emit('responseFromHttpServer-waveform', JSON.stringify({ clientid: data.clientid, dataFromHttpServer: data.dataFromDataClient, pitchFromHttpServer: data.pitchFromDataClient, event: event, duration: duration}));
        ...
        if(Timer5 == 0)
        {
            ...
            socket.broadcast.to('http-floorplan').emit('responseFromHttpServer-floorplan-Pulse', JSON.stringify({ clientid: data.clientid, dataFromHttpServer: avg5samples, pitchFromHttpServer: data.pitchFromDataClient, absLoudLevel: absSoundLevelCalc(avg5samples), duration: duration}));
            Timer5 = 5;
        }   
        ...
        if(Timer20 == 0)  
        {
            setImmediate(() => {        //Don't wanna block the loop for expensive operations
                ...
                socket.broadcast.to('http-floorplan').emit('responseFromHttpServer-floorplan-Circle', JSON.stringify({ clientid: data.clientid, avgSmall: avg30min, avgLarge: avg6hr }));    
            });
            Timer20 = 20;
        }
        ...
        //Soundlist
        if(event != "Blank")
            socket.broadcast.to('http-soundlist').emit('responseFromHttpServer-soundlist', JSON.stringify({ clientid: data.clientid, event: event, eventTimeStamp: eventTimeStamp, peakLoudness: eventPeak, peakFrequency: eventFreq, duration: duration, numOfEventsInDay: numOfEventsInDay}));
