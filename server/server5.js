...
httpListener.sockets.on('connection', function (socket) {
	...
	socket.on('responseFromDataClient', function (data) {
		...
		/*** Emit message...***/
		socket.broadcast.emit('responseFromHttpServer', JSON.stringify({ dataFromHttpServer: data.dataFromDataClient, pitchFromHttpServer: data.pitchFromDataClient, arrayWaveform: arrayWaveform, absLoudLevel: absSoundLevel, event: event, duration: duration, numOfEventsInDay: numOfEventsInDay, clientid: data.clientid, avgSmall: avg10min, avgLarge: avg24hr }));
